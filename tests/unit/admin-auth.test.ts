 ...
 *
 * Kusur şuydu: oturumsuz bir ziyaretçi /admin/kasa'ya gittiğinde MÜŞTERİ
...
const YONETIM = path.join(SRC, 'app/(admin)/admin')
...
describe('middleware — hangi kapıya yönlendiriyor', () => {
  it('⚠️ /admin altındaki sayfalar PERSONEL kapısına gider, müşteri kapısına DEĞİL', () => {
    for (const p of [
      '/admin',
      '/admin/fulfillment',
      '/admin/kasa',
      ...
      expect(to!.split('?')[0], `${p} yanlış kapıya gitti`).toBe('/admin/giris')
  ...
  it('⚠️ /admin/giris KENDİSİ korumasızdır (yoksa sonsuz döngü)', () => {
    /**
     ...
     * oturumsuz ziyaretçi /admin/giris → /admin/giris → … döner ve
     * tarayıcı "çok fazla yönlendirme" hatası verir.
     */
    expect(redirectFor('/admin/giris')).toBeNull()
    expect(redirectFor('/admin/giris?next=/admin/kasa')).toBeNull()
  })

  it('oturum çerezi varsa middleware karışmaz (rol kontrolü sunucuda)', () => {
    expect(redirectFor('/admin/kasa', { session: true })).toBeNull()
  })
...
describe('sayfa yapısı', () => {
  ...
  it('panel sayfaları oturumsuz isteği personel kapısına yolluyor', () => {
    ...
    for (const p of pages) {
      ...
      expect(body, `${path.relative(ROOT, p)} kapıyı işaret etmiyor`).toContain('/admin/giris')
    }
  })
})
...
describe('personel giriş sayfası', () => {
  ...
  it('kendi ucuna gider — müşteri giriş ucuna DEĞİL', () => {
    expect(form).toContain("'/api/v1/auth/admin'")
    expect(form).not.toContain("'/api/v1/auth/login'")
  })

  it('⚠️ açık yönlendirme engeli /admin ile SINIRLI', () => {
    ...
    expect(page).toContain('/^\\/admin\\/[a-zA-Z0-9\\-/_]*$/')
  })
})

// ===========================================================================
describe('personel giriş ucu', () => {
  const route = readCode(path.join(SRC, 'app/api/v1/auth/admin/route.ts'))
...
describe('müşteri akışı bozulmadı', () => {
  it('müşteri giriş/kayıt sayfaları ve uçları yerinde', () => {
    for (const p of [
      'app/(site)/(auth)/giris/page.tsx',
      'app/(site)/(auth)/kayit/page.tsx',
  ...
  it('iki kapı AYNI oturum mekanizmasını kullanıyor', () => {
    ...
    const staff = read(path.join(SRC, 'app/api/v1/auth/admin/route.ts'))
