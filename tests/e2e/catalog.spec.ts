...
test.describe('gerçek katalog', () => {
  test('1-5 · admin katalog ekranı, fiyat düzenleme ve simülatör', async ({ page }) => {
    ...
    await expect(page.getByTestId('catalog-counts')).toContainText('63 fiyat noktası')
    for (const slug of [
      'takipci',
      'begeni',
      'goruntulenme',
    ...
    // ⚠️ Demo platformlar PASİFTİR: panelde "Pasif" etiketiyle görünürler,
    // müşteri katalogunda hiç yer almazlar.
    await expect(page.getByTestId('platform-tiktok')).toContainText('Pasif')
    const publicCatalog = await (await page.request.get('/api/v1/catalog/snapshot')).json()
    expect(publicCatalog.platforms).toHaveLength(1)
