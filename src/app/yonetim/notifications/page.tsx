...
/**
 * /yonetim/notifications — BİLDİRİM İZLEME (Faz 9)
 *
...
export default async function NotificationsPage({
  ...
  const qs = (patch: Record<string, string | undefined>) => {
    ...
    return `/yonetim/notifications?${p.toString()}`
  }
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <Alert
          ...
          href="/yonetim/fulfillment?bucket=review"
          testId="alert-review"
        />
        <Alert
          ...
          href="/yonetim/fulfillment?bucket=new&sort=oldest"
      ...
      <div className="flex flex-wrap items-center gap-2">
        {FILTERS.map((f) => (
          <Link
            key={f.key}
            href={`/yonetim/notifications?filter=${f.key}`}
        ...
        <form action="/yonetim/notifications" className="ml-auto flex items-center gap-2">
              ...
              {data.items.map((n) => (
                <tr key={n.id} data-testid="notification-row" className="hover:bg-ink-50">
                  ...
                  <td className="px-4 py-3">
                    <Link
                      href={`/yonetim/fulfillment?bucket=all&q=${n.orderNo}`}
