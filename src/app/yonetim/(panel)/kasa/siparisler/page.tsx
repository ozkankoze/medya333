...
/**
 * /yonetim/kasa/siparisler — ELLE GİRİLEN GÜNLÜK SİPARİŞ DEFTERİ
 *
 * ⚠️⚠️ BU EKRAN SİTEDEKİ SİPARİŞLERİ GÖSTERMEZ. Gerçek müşteri siparişleri
 * `/yonetim/fulfillment` altındadır: ödeme akışı üretirler, denetim izi
...
export default async function ManualOrdersPage({
  ...
  if (!user) redirect('/yonetim/giris?next=/yonetim/kasa/siparisler')
    ...
    <div className="flex flex-col gap-8">
      ...
      <header className="flex flex-wrap items-end justify-between gap-4">
        ...
        <nav aria-label="Ay seçimi" className="flex items-center gap-2">
          <a href={`/yonetim/kasa/siparisler?y=${prev.y}&m=${prev.m}`} className={navLink}>
            ← Önceki
          </a>
          <a href={`/yonetim/kasa/siparisler?y=${next.y}&m=${next.m}`} className={navLink}>
            Sonraki →
          </a>
        </nav>
      </header>
