...
/**
 * /yonetim/giris — PERSONEL KAPISI
 *
 * ⚠️ BU SAYFA `(panel)` GRUBUNUN DIŞINDADIR ve bu bir zorunluluktur.
 * `src/app/yonetim/(panel)/layout.tsx` oturum yoksa BU sayfaya yönlendirir.
 * Giriş sayfası da o düzenin içinde olsaydı, kendi kendine yönlendiren
 * sonsuz bir döngü doğardı: /yonetim/giris → oturum yok → /yonetim/giris.
 *
 * Rota grubu URL'i DEĞİŞTİRMEZ; /yonetim/fulfillment hâlâ aynı adrestir.
...
export default async function StaffLoginPage({
  ...
  /**
   * ⚠️ AÇIK YÖNLENDİRME (open redirect) ENGELİ. Yalnızca /yonetim altındaki
   ...
   */
  const safeNext =
    typeof next === 'string' && /^\/yonetim\/[a-zA-Z0-9\-/_]*$/.test(next) && next !== '/yonetim/giris'
      ? next
      : '/yonetim/fulfillment'
  ...
  return (
    <div className="mx-auto flex max-w-md flex-col px-5 py-16">
      ...
      {/*
        ...
        kendi kendine açılmaz; bir SUPERADMIN tarafından /yonetim/kullanicilar
