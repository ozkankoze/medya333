...
/**
 * POST /api/v1/auth/admin — PERSONEL GİRİŞİ (/admin/giris formunun ucu)
 *
...
export async function POST(req: NextRequest) {
  ...
  let limit
  try {
    limit = await rateLimit('auth.admin.ip', ipHash)
  } catch (err) {
    return handleUnexpected('auth.admin', err)
  }
  if (!limit.ok) {
  ...
  try {
    ...
    if (ROLE_LEVEL[user.role] < ROLE_LEVEL.SUPPORT) {
       ...
       */
      /**
       * ⚠️ DENETİM KAYDI ADLARI ("auth.yonetim.*") DEĞİŞTİRİLMEDİ.
       * Panel /yonetim'den /admin'e taşındı ama bu dizeler veritabanındaki
       * GEÇMİŞ satırlarda duruyor. Yeni adı kullanmak, aynı olayın iki farklı
       * adla kaydedilmesi ve denetim geçmişinin sessizce ikiye bölünmesi
       * demek olurdu — bir olayı aramak isteyen kişi yarısını bulurdu.
       */
      await writeAudit({
        actorId: user.id,
        actorIpHash: ipHash,
